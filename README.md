# E-portfolio
# Welcome to My Academic Repository 📚

This repository serves as a central hub for my academic work, including projects, assignments, and notes for the following subjects:

## Subjects

1. **Discrete Structure**
   - 📁 [Projects](/Discrete_Structure/Projects)
   - 📁 [Assignments](./DiscreteStructure/Assignments)
   - 📁 [Notes](./DiscreteStructure/Notes)

2. **Digital Logic**
   - 📁 [Projects](./DigitalLogic/Projects)
   - 📁 [Assignments](./DigitalLogic/Assignments)
   - 📁 [Notes](./DigitalLogic/Notes)

3. **Technology Information System**
   - 📁 [Projects](./TechnologyInformationSystem/Projects)
   - 📁 [Assignments](./TechnologyInformationSystem/Assignments)
   - 📁 [Notes](./TechnologyInformationSystem/Notes)

---

## How to Navigate the Repository

- Each subject is organized into separate directories with three main subsections:
  - **Projects:** Contains any coursework-related projects.
  - **Assignments:** Contains submitted and practice assignments.
  - **Notes:** Contains all lecture notes, study materials, or references.

## Contact 📬

If you have any questions or suggestions, feel free to reach out!

- **GitHub:** [IanVictor0331](https://github.com/IanVictor0331)
